...
if (new Date - user.lastmiming < 10000) return await conn.reply(m.chat, `*ESPERA UNOS MINUTOS PARA USAR OTRO COMANDO*`,  m)
  try {
    ...
    if (/webp|image|video/g.test(mime)) {
      ...
      //await conn.reply(m.chat, `${eg}⏳ *CREANDO STICKER, UN MOMENTO...╰⊱💚⊱ *ÉXITO* ⊱💚⊱╮

𝙎𝙀 𝙀𝙎𝙏𝘼 𝗖𝗥𝗘𝗔𝗡𝗗𝗢 𝙎𝙐 𝗘𝗦𝗧𝗜𝗖𝗞𝗘𝗥, 𝙀𝙎𝙋𝙀𝙍𝙀 𝙐𝙉 𝙈𝙊𝙈𝙀𝙉𝙏𝙊 𝙋𝙊𝙍 𝙁𝘼𝙑𝙊𝙍.


𝗬𝗢𝗨 𝗔𝗥𝗘 𝗔 𝗖𝗥𝗘𝗔𝗧𝗜𝗩𝗘 𝗦𝗘𝗘𝗞𝗘𝗥, 𝗦𝗣𝗔𝗥𝗘 𝗔 𝗠𝗢𝗠𝗘𝗡𝗧 𝗙𝗢𝗥 𝗙𝗔𝗩𝗢𝗥.* `, m)
        if (!stiker) {
    ...
    if (!stiker) stiker = e
     ...
     else await conn.reply(m.chat, `╰⊱❗️⊱ *𝙇𝙊 𝙐𝙎𝙊́ 𝙈𝘼𝙇 | 𝙐𝙎𝙀𝘿 𝙄𝙏 𝙒𝙍𝙊𝙉𝙂* ⊱❗️⊱╮\n\n𝙑𝙐𝙀𝙇𝙑𝘼 𝘼 𝙄𝙉𝙏𝙀𝙉𝙏𝘼𝙍 𝙍𝙀𝙎𝙋𝙊𝙉𝘿𝘼 𝘼 𝙐𝙉𝘼 𝙄𝙈𝘼𝙂𝙀𝙉, 𝙑𝙄𝘿𝙀𝙊, 𝙂𝙄𝙁 𝙊 𝙀𝙉𝙇𝘼𝘾𝙀 𝘿𝙀 𝙏𝙄𝙋𝙊 *.jpg* 𝙋𝘼𝙍𝘼 𝙍𝙀𝘼𝙇𝙄𝙕𝘼𝙍 𝙀𝙇 𝙎𝙏𝙄𝘾𝙆𝙀𝙍\n\n𝙏𝙍𝙔 𝘼𝙂𝘼𝙄𝙉 𝙍𝙀𝙎𝙋𝙊𝙉𝘿 𝙏𝙊 𝘼𝙉 𝙄𝙈𝘼𝙂𝙀, 𝙑𝙄𝘿𝙀𝙊, 𝙂𝙄𝙁 𝙊𝙍 𝙇𝙄𝙉𝙆 𝙊𝙁 𝙏𝙔𝙋𝙀 *.jpg* 𝙏𝙊 𝙈𝘼𝙆𝙀 𝙏𝙃𝙀 𝙎𝙏𝙄𝘾𝙆𝙀𝙍*`, m, { contextInfo: { 'forwardingScore': 200, 'isForwarded': false, externalAdReply:{ showAdAttribution: false, title: wm, body: `Canal de WhatsApp`, mediaType: 2, sourceUrl: cnl, thumbnail: imagen1}}}, { quoted: m })
      }
